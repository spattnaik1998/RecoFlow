...
export async function POST(
  ...
  try {
    ...
    // Fetch recommendation for subsequent preference logic
    ...
    // Record preference signal for temporal decay analysis (non-fatal)
    if (rec) {
      serviceClient.from("preference_signals").insert({
        user_id: user.id,
        signal_type: vote,
        book_title: rec.title,
        book_author: rec.author ?? null,
        reason: reason ?? null,
        session_id,
        weight: 1.0,
      }).then(({ error: sigErr }) => {
        if (sigErr) console.warn("[feedback] preference_signals insert:", sigErr.message);
      });
    }

    // "already_read" → permanently block that title
    if (vote === "dislike" && reason === "already_read" && rec) {
      ...
      const blocked = Array.from(new Set([...current.blocked_titles, rec.title]));
      await serviceClient
        .from("user_preferences")
        .upsert(
    ...
    // "like" → add session themes to preferred_themes
    if (vote === "like") {
